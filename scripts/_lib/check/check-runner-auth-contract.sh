...
#       → api DEFAULT_RUNNER_API_KEY == runner API_TOKEN == X
...
run_scenario() {
  local label="$1" set_args="$2" expected_api="$3" expected_runner="$4"
  local out
  out="$(mktemp)"
...
run_scenario "S1 default render" "" "secret_api_token" "secret_api_token"
echo
run_scenario "S2 single source (runner token)" "--set services.runner.env.API_TOKEN=alpha-token" "alpha-token" "alpha-token"
echo
run_scenario "S3 explicit decouple" "--set services.api.env.DEFAULT_RUNNER_API_KEY=beta-api --set services.runner.env.API_TOKEN=gamma-runner" "beta-api" "gamma-runner"
echo
